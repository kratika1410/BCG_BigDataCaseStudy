...
import analytics  # Assuming that your analytics file is named 'analytics.py'
...
def main():
    ...
    # Perform analyses
    analytics.analyze_crashes_with_male_fatalities(spark, charges_df, primary_person_df, analysis1_output)
    analytics.analyze_two_wheelers_booked(spark, units_df, analysis2_output)
    analytics.analyze_top_vehicle_makes_with_driver_death_no_airbag(spark, charges_df, primary_person_df, units_df, analysis3_output)
    analytics.analyze_hit_and_run_licensed_drivers(spark, primary_person_df, charges_df, analysis4_output)
    analytics.analyze_highest_accidents_without_females(spark, primary_person_df, analysis5_output)
    analytics.analyze_top_vehicle_makes_with_injuries(spark, units_df, analysis6_output)
    analytics.analyze_top_ethnicity_by_vehicle_style(spark, units_df, primary_person_df, analysis7_output)
    analytics.analyze_top_zip_codes_with_alcohol_contributions(spark, units_df, primary_person_df, analysis8_output)
    analytics.analyze_vehicles_with_high_damages_no_property_damage(spark, damages_df, units_df, analysis9_output)
    analytics.analyze_speeding_vehicles_with_top_colors_states(spark, charges_df, primary_person_df, units_df, analysis10_output)
